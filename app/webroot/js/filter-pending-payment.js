$(document).ready(function() {
    $('#delete_form').submit(function(){
        var order = $('#modalDeleteOrder').attr('order');
        $("#modalDeleteOrder").modal('hide');
        var formUrl = $(this).attr('action');
        var formType = $(this).attr('method');
        var formData = {'order_id' : order};
        $.ajax({
            type: formType,
            url: formUrl,
            data: formData,
            success: function(data,textStatus,xhr){
                $(".message").text("Order de pago eliminada satifactoriamente!");
                $("#modalMessage").modal('show');
            },
            error: function(xhr,textStatus,error){
                $(".message").text("Ha ocurrido un error al tratar de eliminar la orden de pago!");
                $("#modalMessage").modal('show');
            }
        }); 
...
function deleteOrder(order) {
    ...
    $("#modalDeleteOrder").attr("order", order);
}