$(document).ready(function() {
    ...
    $('#payments_pending').click(function() {
        $('#modalPayments').modal('show');
    });

    $('#cancel_payment_form').submit(function () {
        payment_id = $('#modalCancelPayment').attr('payment');
        $("#modalCancelPayment").modal('hide');
        var formUrl = $(this).attr('action');
        var formType = $(this).attr('method');
        var formData = {'payment_id' : payment_id};
        $.ajax({
            type: formType,
            url: formUrl,
            data: formData,
            success: function(data,textStatus,xhr){
                $(".message").text("El pago se ha anulado exitosamente!");
                $("#modalMessage").modal('show');
            },
            error: function(xhr,textStatus,error){
                $(".message").text("Ha ocurrido un error al tratar de anular el pago!");
                $("#modalMessage").modal('show');
            }
        }); 
...
function cancelPayment(payment){
    ...
    $("#modalCancelPayment").attr("payment", payment);   
}